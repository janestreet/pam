...
let setup_pam_service file auth_file session_file =
  ...
  | `No      ->
    Writer.save_lines file (get_pam_config_lines auth_file session_file)
;;
...
let with_custom_pam_stack ~auth_env ~session_env ~f =
  with_temp_dir (fun dir ->
    ...
    let%bind () =
      Deferred.List.iter ~f:Unix.unlink
        [ pam_file
        ; auth_file
        ; session_file
        ]
...
let ensure_running_as_root () =
  if Unix.getuid () <> 0
  then failwith "You must run this test as root."
;;
