...
let prompt_input msg ~echo =
  let term_set attr =
    Unix.Terminal_io.tcsetattr attr Unix.stdin ~mode:TCSANOW
  in
...
let set_pam_service_item ~ctx item =
  ...
  |> Or_error.bind ~f:(fun () ->
    Pam.pam_set_item ctx ~item_type ~item)
  |> Or_error.bind ~f:(fun () ->
    Pam.pam_get_item ctx ~item_type)
  |> Or_error.map ~f:(fun s ->
...
let command =
  ...
  Command.basic
    ...
    [%map_open
      let user =
        flag "user" (required string) ~doc:"username"
      and service =
        flag "service" (required string) ~doc:"pam service name"
      in
      main ~service ~user
    ]
;;
